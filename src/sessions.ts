...
export const NO_CHOICE: SessionChoice = Object.freeze({ preset: '', compaction: '' })
...
export class SessionChoices {
  ...
  effective(sessionId: string | undefined): SessionChoice {
    if (sessionId === undefined || !isSessionId(sessionId)) return NO_CHOICE
    try {
   ...
   */
  prune(keep: number = MAX_SESSION_CHOICES): number {
    if (keep < 0) return 0
    let names: string[]
    try {
    ...
    const aged: { name: string; at: number }[] = []
 ...
 */
function narrowChoice(raw: unknown): SessionChoice {
  if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) return NO_CHOICE
 ...
 */
function presetValue(raw: unknown): string {
 ...
 */
function instructionValue(raw: unknown): string {
  return typeof raw === 'string' && (raw === '' || isEntryId(raw)) ? raw : ''
}
