 ...
 */
export function buildConfigSchema(factory) {
    ...
    const volatileField = (node) => (typeof node.volatile === 'function' ? node.volatile() : node);
    const index = buildIndexSchema(factory);
    const shape = {};
    for (const [key, node] of Object.entries(index.dict)) {
        shape[key] = key === 'entries' ? volatileField(node.default(builtinEntries())) : volatileField(node);
    }
    shape['compaction'] = volatileField(factory.union([factory.string(), factory.boolean()]).default(true));
 ...
 */
export function compactionSeamEnabled(value) {
    return value !== false;
}
/**
 ...
 */
export function compactionPointer(value) {
