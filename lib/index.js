...
import { activeCompactionOf, activePresetOf, BUILTIN_PROMPTS, buildConfigSchema, builtinEntries, compactionPointer, compactionSeamEnabled, entryIdFor, MAX_ENTRIES, parseEntries, parsePresets, readBuiltinBody, } from './entries.js';
...
export function apply(ctx, config = {}) {
    ...
    const compactionPrompt = installCompactionPrompt(ctx, {
        resolve: (sessionId) => compactionSeamEnabled(liveValue(config.compaction, true))
    ...
    const sync = () => {
        resolved = {
            ...
            compaction: compactionPointer(liveValue(config.compaction, true)),
