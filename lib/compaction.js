 ...
 */
function instructionIndex(messages) {
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        const source = messages[index]?.source;
        if (typeof source === 'object' && source !== null && source.plugin === INSTRUCTION_PLUGIN) {
            return index;
        }
    }
 ...
 */
function withInstruction(original, text) {
    const record = typeof original === 'object' && original !== null ? original : {};
 ...
 */
export function installCompactionPrompt(ctx, host) {
    const state = { matches: 0, replacements: 0, lastReplacedAt: '', characters: 0 };
    ...
    ctx.inject(['llm'], (scoped) => {
        scoped.on('llm/stream', (options, next) => {
            ...
            if (request === undefined || request.purpose !== 'compaction')
                return next();
            state.matches += 1;
            ...
            for (const reference of rendered.escaped) {
                report(`${instruction.id}\u0000${reference}`, `${instruction.id} の本文が ${JSON.stringify(reference)} を参照しています。レジストリが解析できないためリテラルとしてレンダリングしました（この参照を変更するか、いずれかのソースでその名前を登録してください）`);
            }
            try {
                request.messages = [
                ...
                report(`${instruction.id}\u0000frozen`, `圧縮命令 ${instruction.id} は今回のリクエストを置き換えられませんでした（${messageOf(error)}）。DSH 内蔵の命令をそのまま使用しました`);
                return next();
            }
