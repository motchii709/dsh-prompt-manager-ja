 ...
 */
async function readBounded(response, target) {
...
function loadUndici() {
    try {
        const loaded = createRequire(import.meta.url)('undici');
        const candidate = typeof loaded === 'object' && loaded !== null && typeof loaded.fetch === 'function'
            ? loaded
            : undefined;
        return candidate;
    }
...
export function createFetcher(config) {
    ...
    return {
        async get(url, options = {}) {
            ...
            const headers = { 'user-agent': '@lolkda/dsh-prompt-manager' };
            ...
            if (dispatcher !== undefined && undici !== undefined)
            ...
            if (response.status === 304)
                return { status: 304, text: '', etag, contentType };
 ...
 */
export function looksLikeHtml(response) {
